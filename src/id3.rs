...
/// [`Tag`]: Tag
/// [`None`]: Option::None
pub fn parse_tag(bytes: &[u8]) -> Option<Tag> {
    let (_, remaining_bytes) = Header::from_bytes(bytes)?;
    let frames: Vec<Frame> = FrameIter { remaining_bytes }.collect();
    Some(Tag { frames })
}
...
impl Header {
    fn from_bytes(bytes: &[u8]) -> Option<(Self, &[u8])> {
        ...
        match identifier {
            b"ID3" => {
                let version = u16::from_be_bytes(*bytes[0..2].as_array()?);
                let flags = HeaderFlags(*bytes.get(2)?);
                ...
                let size = u32::from_be_bytes(*bytes[3..7].as_array()?);
                ...
                let remaining_bytes = &bytes[7..];
                let remaining_bytes = &remaining_bytes[..size as usize];
...
impl ExtendedHeader {
    fn from_bytes(bytes: &[u8]) -> Option<(Self, &[u8])> {
        ...
        let extended_header_size = u32::from_be_bytes(*bytes[0..4].as_array()?);
        let extended_flags = ExtendedHeaderFlags(u16::from_be_bytes(*bytes[4..6].as_array()?));
        let size_of_padding = u32::from_be_bytes(*bytes[6..10].as_array()?);
        ...
        if extended_flags.crc_data_present() {
            ...
            let extended_header = ExtendedHeader {
                ...
                extended_flags,
                size_of_padding,
                total_frame_crc: Some(u32::from_be_bytes(*bytes[10..14].as_array()?)),
            };

            Some((extended_header, &bytes[14..]))
        } else {
            let extended_header = ExtendedHeader {
                extended_header_size,
                extended_flags,
...
impl Frame {
    fn from_bytes(bytes: &[u8]) -> Option<(Self, &[u8])> {
        ...
        // must be at least 1.
        if header.size as usize > bytes.len() || header.size < 1 {
            return None;
        }
    ...
    fn from_bytes(bytes: &[u8]) -> Option<(Self, &[u8])> {
        ...
        Some((
            FrameHeader {
                ...
                size: u32::from_be_bytes(*bytes[4..8].as_array()?),
